...
export default function SideNav() {
  ...
  const iconClass = "w-6 h-6 text-black dark:text-white";
  const pages = [
  ...
  return (
    <nav className="w-60 h-full bg-blue-50 dark:bg-gray-900 fixed top-0 left-0 z-10 flex inline-flex flex-col items-center justify-start gap-10 px-4 py-8">
      ...
      <ul className="flex flex-col justify-center items-start p-6 border-t w-full pt-10 border-deep-blue dark:border-white gap-8 font-inter font-bold text-lg">
        {pages.map((page) => (
          <li 
            key={page.name} 
          >
            <Link href={page.href} className="flex inline-flex gap-4 items-center">
              {page.icon}
              <div className={classNames(pathname === page.href ? "text-black border-b-2 border-deep-blue dark:border-white" : "text-deep-blue", "hover:text-sky-dancer dark:text-white cursor-pointer transition-colors duration-300")}>
                {page.name}
              </div>
            </Link>
          </li>
